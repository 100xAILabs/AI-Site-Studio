...
"""
        try:
            ...
            raw = await ai_service._generate_content(frontend_prompt, feature_name="code_assistant")
...
"""
        try:
            raw = await ai_service._generate_content(backend_prompt, feature_name="code_assistant")
            raw = raw.strip()
...
"""
        try:
            raw = await ai_service._generate_content(db_prompt, feature_name="code_assistant")
            raw = raw.strip()
            if raw.startswith("```python"):
                raw = raw[9:]
            elif raw.startswith("```"):
